...
$(document).ready(function () {
    ...
    function dpOnclose(dateText, dpInst){ 
    	//var _this = $(this);
    	//alert("closing dp:" + $(this));
    ...
    function showChildGrid(parentRowID, parentRowKey) {
        ...
        var result = $.grep(_locData, function(element, index) {
   			return (element.keyId === parentRowKey);
		});
        var childGridURL =[];
        if(result.length > 0)
         	childGridURL = result[0].taskList.task;
        ...
        $("#" + childGridID).jqGrid({
            ...
            page: 1,
            colModel: [
                { label: 'TaskId', name: 'id', key: true, editable: false, hidden:true },
                { label: 'Name', name: 'name', width: 150,editable: true },
                { label: 'description', name: 'description', width: 200,editable: true },
                { label: 'prevStatus', name: 't_prevState',jsonmap :'status.prevState', width: 50,editable: false, formatter:fillDefaultStatus},
                { label: 'Status', name: 't_currentState',jsonmap: 'status.currentState',edittype: "select", width: 50,editable: true, 
                	editoptions: {
                ...
                { label: 'wlInf', name: 't_wlInf',jsonmap : 'metric.estimate.wlInf', width: 50,editable: true,
                	edittype: "select",
                    editoptions: {
                        value: getWlValues
                    },
                    editrules:{
                    	custom_func: validateWLInf,
                    	custom: true,
                    	required:true
                    }
                },
                { label: 'wlSup',name:'t_wlSup', jsonmap : 'metric.estimate.wlSup', width: 50,editable: true,formatter: 'integer',
                	edittype: "select",
                    editoptions: {
                        value: getWlValues
                    },
                    editrules:{
                    	custom_func: validateWLSup,
                    	custom: true,
                    	required:true
                    }
                },
                { label: 'startDate', name: 't_startDate',jsonmap: 'metric.estimate.startDate', width: 80,editable: true,
                	edittype:"text",
                    editoptions: {
                        dataInit: function(element){setDatePicker(element,'TstartDate_datePicker')}
                    },
                    editrules:{
                    	custom_func: validateStartDate,
                    	custom: true,
                    	required:true
                    }
                },
                { label: 'endDate', name: 't_endDate',jsonmap: 'metric.estimate.endDate', width: 80,editable: true,
                	edittype:"text",
                    editoptions: {
                        dataInit: function(element){setDatePicker(element,'TendDate_datePicker')}
                    },
                    editrules:{
                    	custom_func: validateEndDate,
                    	custom: true,
                    	required:true
                    }
                }   
            ...
            loadComplete: checkgridComplete,
        ...
        $("#" + childGridID).navButtonAdd("#" + childGridPagerID,
                {
                    ...
                    caption: "Copy task",
                    position: "last",
                    onClickButton: navButtonCopyTask
                });
    ...
    $("#jqGrid").jqGrid({
		...
		width: 1100,
        colModel: [
            { label: '#Sprint', name: 'keyId', width: 75, key:true },
            { label: 'Name', name: 'name', width: 90, editable: true },
            { label: 'startDate', name: 'startDate',  
                ...
                editoptions: {
                    ...
                    dataInit:  function(element){setDatePicker(element,'startDate_datePicker')}
                }
            
            
            },
            { label: 'endDate', name: 'endDate', width: 80, editable: true,
            	edittype:"text",
                editoptions: {
                    dataInit: function(element){setDatePicker(element,'endDate_datePicker')}
                }
            
            },
            {
            	label:'Budget',editable:false,name:'Budget'
            }
